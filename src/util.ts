...
export const isValidTag = (tag: Tag, requireName = true): boolean => {
    ...
    switch (type) {
        ...
        case TagType.FLOAT: {
            ...
            return (
                Number.isNaN(val) ||
                (val >= 1.175494351e-38 && val <= 3.402823466e38)
            )
        }
        case TagType.DOUBLE: {
        ...
        case TagType.LIST: {
            ...
            return val.every(
                tag => tag.type === val[0].type && isValidTag(tag, false)
            )
        }
        case TagType.COMPOUND: {
            ...
            return (
                val.length === new Set(val.map(tag => tag.name)).size &&
                val.every(tag => isValidTag(tag))
            )
        }
        case TagType.INT_ARRAY: {
            const val = value as ReturnType<typeof intArray>['value']
            return val.every(
                num => num >= -2_147_483_648 && num <= 2_147_483_647
            )
        }
